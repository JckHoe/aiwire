...
test-openai:
	$(GO) test -v -tags=openai -count=1 ./integration/...

test-openrouter:
	$(GO) test -v -tags=openrouter -count=1 ./integration/...

test-zai:
	$(GO) test -v -tags=zai -count=1 ./integration/...

test-cerebras:
	$(GO) test -v -tags=cerebras -count=1 ./integration/...

test-usage:
	$(GO) test -v -tags=usage -count=1 ./integration/...
